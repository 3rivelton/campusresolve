...
public class LoginController {
    ...
    public String loginServidor(String matricula, String senha) {
        try {
            this.servidorLogado  = (Servidor) ManagerDao.getCurrentInstance()
                    .read("select p from Servidor p" + " where p.matricula = '" + matricula + "' and p.senha = '" + senha+"'", 
    ...
    public String loginUsuario(String matricula, String senha) {
        try {
            this.usuarioLogado  = (Usuario) ManagerDao.getCurrentInstance()
                    .read("select p from Usuario p" + " where p.matricula = '" + matricula + "' and p.senha = '" + senha+"'", 
                            Usuario.class).get(0);                        
            return "indexUsuario";
        } catch (Exception e) {            
            e.printStackTrace();
